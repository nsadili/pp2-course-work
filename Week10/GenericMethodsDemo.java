public class GenericMethodsDemo {
    ...
    static <T> void printAll(T[] arr) {// T and Obj can acces everything so cause error

        // for (T x : arr)
        // System.out.println(x + " ");
        // System.out.println();

        printAll(arr, 0, arr.length - 1); // we call it there in case if we want to change smt change in 1
    ...
    static void printAll(Task[] arr) {
        for (Task t : arr)
            System.out.println(t); // for(0-length)
    }

    static <T> T mid(T[] arr) {// in diamond we declare then use (T)
