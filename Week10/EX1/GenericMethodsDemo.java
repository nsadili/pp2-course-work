...
public class GenericMethodsDemo {
    ...
    // static public void printAll(Object[] arr){
    //     for (Object el : arr)
    //         System.out.print(el + " ");
    //     System.out.println();
    // }
    ...
    static <T> void printAll(T[] arr, int lowerBound, int upperBound){
        if (arr.length == 0) throw new IllegalArgumentException("Input array is emptry");
        if(upperBound >= arr.length || lowerBound < 0 || upperBound < 0 || lowerBound >= arr.length) 
    ...
    static <T> T middle(T[] arr){
        if (arr.length == 0)
            throw new ArrayIndexOutOfBoundsException("Input array cannot be empty");
