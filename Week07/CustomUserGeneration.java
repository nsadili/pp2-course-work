...
public class CustomUserGeneration {
    public static void main(String[] args) {
        try {
            ...
            e.printStackTrace();
        } catch(InvalidRolesArgumentException e) {
            e.printStackTrace();
        }

    }

    public static void createUser(String username, String password, String[] roles) throws InvalidUsernameException, InvalidPasswordException, InvalidRolesArgumentException {
       if (username == null || username.length() < 3)
        ...
        if(roles == null || roles.length == 0)
            throw new InvalidRolesArgumentException("roles cannot be null or an empty array");
    }
}